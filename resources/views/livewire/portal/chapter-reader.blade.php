    ...
    <header class="bg-navbg relative py-8 border-b-2 border-purple-500/20">
        ...
        <div class="max-w-4xl mx-auto px-4">
            ...
            <nav class="mb-4" aria-label="Breadcrumb">
                <a href="{{ route($this->bookLink, $this->book->id) }}" wire:navigate class="text-white/80 hover:text-violet-400 font-serif text-sm transition-colors inline-flex items-center gap-2" aria-label="Return to {{ $this->book->title }} book page">
            ...
            <nav class="mt-12 pt-8 border-t-2 border-purple-500/20" aria-label="Chapter navigation">
                <div class="flex items-center justify-between gap-4">
                    @if($this->previousChapter)
                        <a href="{{ route($this->link, ['bookId' => $this->bookId, 'chapterNumber' => $this->previousChapter->chapter_number]) }}" wire:navigate class="group flex-1 relative bg-white/80 dark:bg-accent/30 backdrop-blur-sm rounded-none border-2 border-purple-500/30 dark:border-purple-400/20 hover:border-purple-500 hover:shadow-xl dark:hover:shadow-purple-500/10 transition-all duration-500 overflow-hidden p-6" aria-label="Previous chapter: {{ $this->previousChapter->title }}">
                    ...
                    @if($this->nextChapter)
                        <a href="{{ route($this->link, ['bookId' => $this->bookId, 'chapterNumber' => $this->nextChapter->chapter_number]) }}" wire:navigate class="group flex-1 relative bg-white/80 dark:bg-accent/30 backdrop-blur-sm rounded-none border-2 border-violet-400/30 dark:border-violet-400/20 hover:border-violet-500 hover:shadow-xl dark:hover:shadow-violet-500/10 transition-all duration-500 overflow-hidden p-6" aria-label="Next chapter: {{ $this->nextChapter->title }}">
